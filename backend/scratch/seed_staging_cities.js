...
async function seed() {
  console.log("Starting seeding of cities for all states on staging...");
  
  // Fetch all states from db to get correct UUIDs
  const dbStates = await State.findAll();
  console.log(`Found ${dbStates.length} states in the database.`);
  ...
  for (const dbState of dbStates) {
    ...
    console.log(`Seeding ${cityNameList.length} cities for state: ${dbState.name} (ID: ${dbState.id})`);
